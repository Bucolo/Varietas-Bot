...
EMBED_COLOR = discord.Colour.material_green()
SKIP_EMOJI = "⏭️"
BACK_EMOJI = "◀️"
DC_EMOJI = ""
DEL_AFTER_TIME = 5
...
class Music(commands.Cog):
    ...
    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.CommandInvokeError):
            embed = discord.Embed(
                ...
                ```
                """,
                colour=discord.Colour.material_red()
            )
    ...
    def rq_check(ctx):
        return (
            ctx.author.id == ctx.bot.lavalink.player_manager.get(
                ctx.guild.id).current.requester)
    ...
    async def skip(self, ctx):
        ...
        if ctx.author.id == player.current.requester or len(
                self.skip_votes) >= self.required(ctx):
            await player.skip()
            embed = discord.Embed(title="⏭ Skipped.", color=EMBED_COLOR)
            await ctx.send(embed=embed, delete_after=DEL_AFTER_TIME)
        else:
            embed = discord.Embed(
                title=f"{ctx.author.mention} has voted to skip the song. ({len(self.skip_votes)}/{self.required(ctx)})")
    ...
    async def stop(self, ctx):
        ...
        self.stop_votes.add(ctx.author)

        if ctx.author.id == player.current.requester or len(
                self.stop_votes) >= self.required(ctx):
            ...
            embed = discord.Embed(
                title=f"{ctx.author.mention} has voted to stop the player. ({len(self.stop_votes)}/{self.required(ctx)})",
                colour=EMBED_COLOR)
            await ctx.send(embed=embed, delete_after=DEL_AFTER_TIME)
    ...
    async def now(self, ctx):
        ...
        embed.set_footer(
            text="Join our support server! discord.gg/varietas",
            icon_url=self.bot.user.avatar_url_as(static_format="png")
        )
    ...
    async def pause(self, ctx):
        ...
        if player.paused:
            ...
            self.pause_votes.add(ctx.author)

            if ctx.author.id == player.current.requester or len(
                    self.pause_votes) >= self.required(ctx):
                ...
                embed = discord.Embed(
                    title=f"{ctx.author.mention} has voted to pause the current song. ({len(self.pause_votes)}/{self.required(ctx)})",
                    colour=EMBED_COLOR)
    ...
    async def find(self, ctx, *, query):
        ...
        if not query.startswith(
                "ytsearch:") and not query.startswith("scsearch:"):
    ...
    async def disconnect(self, ctx):
        ...
        self.dc_votes.add(ctx.author.id)

        if len(self.dc_votes) >= self.required(ctx):
            self.dc_votes.clear()
            player.queue.clear()
            await player.stop()
            ...
            embed = discord.Embed(
                ...
                color=EMBED_COLOR)
            embed.set_footer(
                text="Thanks For using Varietas! Join our support server! discord.gg/varietas",
                icon_url=self.bot.user.avatar_url_as(
                    static_format="png"))
            await ctx.send(embed=embed, delete_after=DEL_AFTER_TIME)
        else:
            embed = discord.Embed(
                title=f"{ctx.author.mention} has voted to disconnect the player. ({len(self.dc_votes)}/{self.required(ctx)})",
                colour=EMBED_COLOR)
    ...
    async def ensure_voice(self, ctx):
        ...
        should_connect = ctx.command.name in (
            "play", "playlist", "add", "load", "show", "remove")
    ...
    async def playlist_add(self, ctx, *, query: str):
        query = query.strip('<>')
        if not url_rx.match(query):
            query = f'ytsearch:{query}'
        track = await self.bot.lavalink.node_manager.find_ideal_node().get_tracks(query)
        try:
            track = track[0]
        except BaseException:
            pass
        if track is None:
            return await ctx.send("Invalid Youtube Link!", delete_after=15)

        playlistSize = await playlist.DBPlaylist(self.bot.db).countSongs(ctx.author.id, "liked")
        if playlistSize >= 25:
            return await ctx.send("Your playlist (\"`liked`\") is full! (Max songs: `25`)", delete_after=20)
        track_title = track["tracks"][0]["info"]["title"]
        track_uri = track["tracks"][0]["info"]["uri"]
        track_identifier = track["tracks"][0]["info"]["identifier"]
        track_thumb = f"https://img.youtube.com/vi/{track_identifier}/mqdefault.jpg"
        await playlist.DBPlaylist(self.bot.db).add(ctx.author.id, "liked", track_title, track_uri)

        embed = discord.Embed(
            title="Song successfully added to your playlist!",
        ...
        embed.add_field(
            name="Playlist size: ",
            value=f"`{1 if playlistSize == 1 else playlistSize+1}/25`",
            inline=True)
        embed.set_image(url=track_thumb)
    ...
    async def playlist_show(self, ctx):
        playlistContent = await playlist.DBPlaylist(self.bot.db).display(ctx.author.id, "liked")

        if len(playlistContent) <= 0:
            return await ctx.send("Your playlist (`liked`) is empty!", delete_after=15)
    ...
    async def playlist_remove(self, ctx, index):
        ...
        playlistContent = await playlist.DBPlaylist(self.bot.db).display(ctx.author.id, "liked")
        if len(playlistContent) <= 0:
            return await ctx.channel.send(f"Your playlist (`liked`) is empty!", delete_after=15)

        if index < 0 or index > (len(playlistContent) - 1):
            return await ctx.channel.send(f"The index is available!", delete_after=15)
        await playlist.DBPlaylist(self.bot.db).remove(ctx.author.id, "liked", playlistContent[index][3])
        ...
        embed = discord.Embed(
            ...
            ")**",
            color=discord.Colour.material_green())
    ...
    @playlist.command(
        ...
        aliases=[],
        usage="",
        description="Load all songs from your playlist into the current queue")
    ...
    async def playlist_load(self, ctx):
        ...
        for i in playlistContent:
            print(i)
            await self.play(ctx=ctx, query=i[2])
...
class ReactionMenu(menus.Menu):
    ...
    @menus.button(SKIP_EMOJI.strip("<>"))
    async def on_skip_emoji(self, payload):
